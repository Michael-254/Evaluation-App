                            ...
                            <form method="POST" action="{{route('sectionFour.store')}}">
                                ...
                                <div class="row">
                                    <div class="form-group w-full">
                                        ...
                                        <textarea name="sup_works_well" class="summernote bg-green-500">{{$info->sup_works_well ?? ''}}{{ old('sup_works_well') }}</textarea>
                                    </div>
                                    <div class="form-group w-full">
                                        <p class="font-sans text-green-700 mb-2">Supervisor: Need for improvement</p>
                                        <textarea name="sup_needs_improvement" class="summernote">{{$info->sup_needs_improvement ?? ''}}{{ old('sup_needs_improvement') }}</textarea>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="form-group w-full">
                                        <p class="font-sans text-green-700 mb-2">Other parts of the Organization: What has worked well</p>
                                        <textarea name="org_works_well" class="summernote bg-green-500">{{$info->org_works_well ?? ''}}{{ old('org_works_well') }}</textarea>
                                    </div>
                                    <div class="form-group w-full">
                                        <p class="font-sans text-green-700 mb-2">Other parts of the Organization: Need for improvement</p>
                                        <textarea name="org_needs_improvement" class="summernote">{{$info->org_needs_improvement ?? ''}}{{ old('org_needs_improvement') }}</textarea>
                                    </div>
                                </div>
